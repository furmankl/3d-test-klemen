<template>
  <section class="panel">
    ...
    <form v-if="object" class="stack" @submit.prevent="submit">
      <label class="field"><span>Ime</span><input v-model.trim="draft.name" type="text" /></label>
      <div class="two-columns">
        <label class="field"><span>Tip</span><input :value="object.type" disabled /></label>
        <label class="field"><span>Barva</span><input v-model="draft.color" type="color" /></label>
      </div>
      <label class="checkbox-field"
        ><input v-model="draft.visible" type="checkbox" /><span
      ...
      <button class="button button--primary" type="submit" disabled>Shrani spremembe</button>
      <p class="todo-note">
...
const emit = defineEmits<{ save: [id: string, patch: SceneObjectPatch] }>()
